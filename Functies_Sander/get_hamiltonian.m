...
% * Different Heisenberg XXZ and XXX models
    ...
    elseif strcmp('Hubbard_two_site', type)
        ...
        t_U1_1_trees0 = {[C1 C0 C0 C1] [C1 C0 C0 C1] [C2 C0 C1 C1] [C2 C0 C1 C1] [C1 C1 C0 C2] [C1 C1 C0 C2] [C2 C1 C1 C2] [C2 C1 C1 C2]};
        t_U1_2_trees = {[0 up up 0] [0 down down 0] [0 0 up down] [0 0 down up] [down up 0 0] [up down 0 0] [up 0 0 up] [down 0 0 down]};
        t_U1_2_trees0 = {[up 0 0 up] [down 0 0 down] [0 0 down up] [0 0 up down] [up down 0 0] [down up 0 0] [0 up up 0] [0 down down 0]};
        len = length(fusion_trees);
        H_vars = zeros(1, len);
        ...
        for i = 1:len
            ...
            for k = 1:length(t_U1_1_trees)
                if all(t_U1_1_trees{k} == c1) && all(t_U1_2_trees{k} == c2)
                    H_vars(i) = H_vars(i) - t/2;
                end
                if all(t_U1_1_trees0{k} == c1) && all(t_U1_2_trees0{k} == c2)
                    H_vars(i) = H_vars(i) - t/2;
                end
            end
            %if ismember(c, t_trees)
