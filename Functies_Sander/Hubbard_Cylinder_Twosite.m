...
mps = UniformMps.randnc([pspace pspace], [vspace vspace]);
...
alg = IDmrg2('dynamical_tols', true, 'which', 'smallestreal', 'tol', 10^(-3), 'tol_max', 10^(-3), 'trunc', {'TruncBelow', 10^(-5)}, 'maxiter', 200, 'verbosity', Verbosity.iter, 'name', 'Hubbard_Cylinder_Twosite', 'doSave', true, 'saveIterations', 1);
[gs_mps, gs_energy] = fixedpoint(alg, H1, mps);
...
numbers = get_filling(gs_mps, pspace, trivspace, 1, true);
